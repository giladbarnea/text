...
# Each tuple is (exact match, lenient match, page number)
# Rationale behind lenient match: it's insensitive to numbering, whitespace, punctuation, case, and requires only a prefix match.
# This is WIP and we want to know even when a heading "mostly" matches.
EXPECTED_TOC: list[tuple[Text, re.Pattern | None, Page]] = [
    ("Abstract", None, 8),
    ("Introduction", None, 12),
    ("1. Fibromyalgia", re_compile(r"^(1\. ?)?Fibromyalgia"), 13),
    (
        "2. Stress, Stress-Related Medical Conditions, and Psychological Interventions",
        re_compile(r"^(2\. ?)?Stress, Stress-Related Medical Conditions"),
        14,
    ),
    (
        "2.1. Stress and Rheumatic Diseases",
        re_compile(r"^(2\.1\. ?)?Stress and Rheumatic Diseases"),
        14,
    ),
    (
        "2.2. Stress and Chronic Pain Conditions",
        re_compile(r"^(2\.2\. ?)?Stress and Chronic Pain Conditions"),
        15,
    ),
    (
        "3. Cognitive Behavioral Therapy",
        re_compile(r"^(3\. ?)?Cognitive Behavioral Therapy"),
        16,
    ),
    (
        "3.1. CBT Studies among FM Patients",
        re_compile(r"^(3\.1 ?)?CBT Studies among FM Patients"),
        18,
    ),
    ("4. Mindfulness", re_compile(r"^(4\. ?)?Mindfulness"), 20),
    (
        "4.1. Mindfulness-Based Stress Reduction (MBSR)",
        re_compile(r"^(4\.1 ?)?Mindfulness.Based Stress Reduction .?MBSR.?"),
        21,
    ),
    (
        "4.2. MBSR Studies among FM Patients",
        re_compile(r"^(4\.2 ?)?MBSR Studies among FM Patients"),
        22,
    ),
    (
        "5. Comparison between CBT and MBSR for FM",
        re_compile(r"^(5\. ?)?Comparison between CBT and MBSR for FM"),
        23,
    ),
    (
        "6. Internet Based Psychological Therapy for FM",
        re_compile(r"^(6\. ?)?Internet Based Psychological Therapy for FM"),
        24,
    ),
    (
        "7. Adapting Generic Treatment Protocols for FM Patients",
        re_compile(r"^(7\. ?)?Adapting Generic Treatment Protocols for FM Patients"),
        26,
    ),
    (
        "8. Mechanisms of Change in FM Psychotherapy",
        re_compile(r"^(8\. ?)?Mechanisms of Change in FM Psychotherapy"),
        27,
    ),
    (
    ...
    (
        ...
        re_compile(r"(8\.2 ?)?Pain Catastrophizing as a Mechanism of Change"),
        30,
    ),
    (
        "8.3. Emotion regulation as a Mechanism of Change",
        re_compile(r"(8\.3 ?)?Emotion regulation as a Mechanism of Change"),
        31,
    ),
    ...
    (
        ...
        re_compile(r"^(1\. ?)?Participants and Procedure"),
        36,
    ),
    (
        "2. Randomization and Measurement",
        re_compile(r"^(2\. ?)?Randomization and Measurement"),
        40,
    ),
    ("3. Treatment protocols", re_compile(r"^(3\. ?)?Treatment protocols"), 40),
    ("3.1. MBSR protocol", re_compile(r"^(3\.1 ?)?MBSR protocol"), 40),
    ("3.2. CBT protocol", re_compile(r"^(3\.2 ?)?CBT protocol"), 41),
    ("4. Measures", re_compile(r"^(4\. ?)?Measures"), 41),
    ("Results", None, 44),
    (
        "1. Pre- to Post-treatment Differences Between the CBT, MBSR and WL groups",
        re_compile(r"^(1\. ?)?Pre.? to Post-treatment Differences Between the CBT"),
        44,
    ),
    (
        "2. Long-Term Effectiveness of CBT and MBSR",
        re_compile(r"^(2\. ?)?Long.Term Effectiveness of CBT and MBSR"),
        49,
    ),
    (
        "2.1. Long-Term Effectiveness of MBSR",
        re_compile(r"^(2\.1 ?)?Long.Term Effectiveness of MBSR"),
        49,
    ),
    (
        "2.2. Long-Term Effectiveness of CBT",
        re_compile(r"^(2\.2 ?)?Long.Term Effectiveness of CBT"),
        51,
    ),
    (
    ...
    (
        ...
        re_compile(r"^(3\.1 ?)?Pre.? to Post.treatment Differences Between the MBSR"),
        52,
    ),
    (
        "3.2. Long-Term Effectiveness of MBSR and MBSR ZOOM",
        re_compile(r"^(3\.2 ?)?Long.Term Effectiveness of MBSR and MBSR ZOOM"),
        56,
    ),
    (
        "3.3. Pre- to Post-treatment Differences Between the CBT, CBT ZOOM and WL groups",
        re_compile(r"^(3\.3 ?)?Pre.? to Post.treatment Differences Between the CBT"),
        59,
    ),
    (
        "3.4. Long-Term Effectiveness of CBT and CBT ZOOM",
        re_compile(r"^(3\.4 ?)?Long.Term Effectiveness of CBT and CBT ZOOM"),
        62,
    ),
    (
        "4. Potentials Mechanisms of Change",
        re_compile(r"^(4\. ?)?Potentials Mechanisms of Change"),
        64,
    ),
    ("General Discussion", None, 69),
    (
        "1. The Effectiveness of CBT and MBSR for FM Patients",
        re_compile(r"^(1\. ?)?The Effectiveness of CBT and MBSR for FM Patients"),
        70,
    ),
    (
        "1.1. The Effectiveness of MBSR for FM Patients",
        re_compile(r"^(1\.1 ?)?The Effectiveness of MBSR for FM Patients"),
        70,
    ),
    (
        "1.2. The Effectiveness of CBT for FM Patients",
        re_compile(r"^(1\.2 ?)?The Effectiveness of CBT for FM Patients"),
        72,
    ),
    (
        "1.3. The Comparison between CBT and MBSR",
        re_compile(r"^(1\.3 ?)?The Comparison between CBT and MBSR"),
        72,
    ),
    (
        "2. Differences Between Face-to-Face and Online Therapy",
        re_compile(r"^(2\. ?)?Differences Between Face.to.face and Online Therapy"),
        74,
    ),
    (
        "3. Potentials Mechanisms of Change in MBSR and CBT",
        re_compile(r"^(3\. ?)?Potentials Mechanisms of Change in MBSR and CBT"),
        78,
    ),
    (
        "1.1. The Relationship between Potential Therapeutic Mechanisms Outcome Measures",
        re_compile(r"^(1\.1 ?)?The Relationship between Potential Therapeutic"),
        79,
    ),
    (
        "1.2. Potential Therapeutic Mechanisms in MBSR",
        re_compile(r"^(1\.2 ?)?Potential Therapeutic Mechanisms in MBSR"),
        79,
    ),
    (
        "1.3. Potential Therapeutic Mechanisms in CBT",
        re_compile(r"^(1\.3 ?)?Potential Therapeutic Mechanisms in CBT"),
        82,
    ),
    (
